...
const BlogPostPage: React.FC = () => {
  ...
  useEffect(() => {
    if (post) {
      document.title = `${post.title} | Marek Studnička Blog`;
      // Optionally set meta description
      const metaDescriptionTag = document.querySelector('meta[name="description"]');
      if (metaDescriptionTag) {
        metaDescriptionTag.setAttribute('content', post.description);
      } else {
        const newMeta = document.createElement('meta');
        newMeta.name = "description";
        newMeta.content = post.description;
        document.head.appendChild(newMeta);
        // Store it to remove on unmount if you add more metas
      }
    }
